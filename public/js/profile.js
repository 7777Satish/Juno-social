document.addEventListener("DOMContentLoaded", function () {
    ...
    var shouldAnyMoreRequestsBeMade = true;
    window.addEventListener('scroll', async () => {

        if(shouldAnyMoreRequestsBeMade && anymoreposts && Math.abs(window.scrollY+innerHeight-thereadend.offsetTop)<500){
            shouldAnyMoreRequestsBeMade = false;
            var posts = await fetch('/api/postsByUsername?page=' + page+'&username='+window.location.pathname.split('/')[2]);
    ...
    <div class="top">
        ...
        <div class="right">
            <h3><a href="/p/${element.username}">${element.fullname}</a> <span>@${element.username}</span></h3>
            <span>${new Date(element.date).toDateString()}</span>
        </div>
    </div>
    <a href="/p/${element.username}/post/${element.postid}">
        <div class="content">
...
`;
            ...
            page++;
            shouldAnyMoreRequestsBeMade = true;
        }
    });
});
